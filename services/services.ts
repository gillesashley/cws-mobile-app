...
export const useMxPointTransaction = (data: typeof zPointTransaction._type, options?: SWROptions) => {
    ...
    const req = useSWR(
        ...
        ({ url, data }) => {
            ...
            return axios.axiosInstance.post(buildApiUrl(`/point-transactions`), _data).then((r) => r.data);
        },
        options,
    );
    return req;
...
export const useApi = () => {
    ...
    return {
        axiosInstance,
        getRegions: (options?: SWROptions) =>
            useSWR<Region[], AxiosError>("/regions?include=constituencies", (key) => axiosInstance.get(key).then((r) => r.data.data), options),
        getConstituencies: (options?: SWROptions) =>
            useSWR<Constituency[], AxiosError>("/constituencies", (key) => axiosInstance.get(key).then((r) => r.data.data), options),
        getCampaignsConstituency: (options?: SWROptions) =>
            useSWR<CampaignMessage[], AxiosError>(
                "/campaign-messages",
                () =>
                    axiosInstance
                        .get("/campaign-messages?sort=-created_at&include=user_liked&filter[constituency_id]=" + auth.user?.constituency_id)
                        .then((r) => r.data.data),
                options,
            ),
        getCampaignsRegional: (options?: SWROptions) =>
            useSWR<CampaignMessage[], AxiosError>(
                "/campaign-messages/regional",
                () => axiosInstance.get("/campaign-messages?sort=-created_at&include=user_liked&filter[regional]").then((r) => r.data.data),
                options,
            ),
        getCampaignsNational: (options?: SWROptions) =>
            useSWR<CampaignMessage[], AxiosError>(
                "/campaign-messages/national",
                () => axiosInstance.get("/campaign-messages?sort=-created_at&include=user_liked&filter[national]").then((r) => r.data.data),
                options,
            ),
        getUserBalance: (options?: SWROptions) =>
            useSWR<{ balance: number }, AxiosError>("/user-balance", (key) => axiosInstance.get(key).then((r) => r.data), options),
        ...
        mxWithdrawals: (options?: SWROptions) =>
            useSWRMutation("/reward-withdrawals", (url, { arg }: { arg: { amount: number } }) =>
                axiosInstance.post(url, arg).then((d) => zPointWithdrawals.parse(d.data)),
            ),
        getPoints: (options?: SWROptions) => useSWR<PointsData, AxiosError>("/points", (key) => axiosInstance.get(key).then((r) => r.data), options),
        getUserProfile: (options?: SWROptions) =>
            useSWR<UserProfile, AxiosError>("/user-profile", (key) => axiosInstance.get(key).then((r) => r.data.data), options),
        mxUpdateUserProfile: (options?: SWROptions) =>
            useSWRMutation<UserProfile & { token: string | undefined }, AxiosError>(
                "/user-profile",
                (url, { arg }: { arg: typeof zNewUserProfile._type }) => axiosInstance.patch(url, arg) as any,
                options,
            ),
        getConstituencyBanners: (options?: SWROptions) =>
            useSWR<Banner[], AxiosError>(
                `/banners?filter[bannerable_id]=${auth.user?.constituency_id}&filter[bannerable_type]=Constituency`,
                (key) => axiosInstance.get(key).then((r) => r.data.data),
                options,
            ),
        getRegionBanners: (options?: SWROptions) =>
            useSWR<Banner[], AxiosError>(
                `/banners?filter[bannerable_id]=${auth.user?.region_id}&filter[bannerable_type]=Region`,
                (key) => axiosInstance.get(key).then((r) => r.data.data),
                options,
            ),
        getNationalBanners: (options?: SWROptions) =>
            useSWR<Banner[], AxiosError>(
                `/banners?filter[bannerable_id]=null&filter[bannerable_type]=null`,
                (key) => axiosInstance.get(key).then((r) => r.data.data),
                options,
            ),
    };
