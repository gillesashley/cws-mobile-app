...
import { Alert, Linking, Modal, Share, StyleSheet, TouchableOpacity, View } from "react-native";
...
export function EnhancedShareButton({ campaignId, shareableUrl, initialShares, onShareSuccess }: EnhancedShareButtonProps) {
    const [isShareModalVisible, setIsShareModalVisible] = useState(false);
    const [shares, setShares] = useState(initialShares);
    const { user } = useAuthContext();

    const backgroundColor = useThemeColor({}, "background");
    const iconColor = useThemeColor({ light: "#757575", dark: "#A0A0A0" }, "text");
        ...
        try {
            ...
            switch (platform) {
                ...
                case "facebook":
                    ...
                    break;
                case "twitter":
                    url = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareableUrl)}`;
                    break;
            }
            ...
            Alert.alert("Success", `You earned ${response.data.points_awarded} points for sharing this post!`);
        } catch (error) {
    ...
    return (
        <>
            <TouchableOpacity style={styles.statItem} onPress={() => setIsShareModalVisible(true)}>
