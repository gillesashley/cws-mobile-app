...
export default function ShareModal({ isVisible, onClose, onShare, postId }: ShareModalProps) {
    const { user } = useAuthContext();
    const backgroundColor = useThemeColor({}, "background");

    const handleShare = async (platform: string) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/campaign-messages/${postId}/share`, {
                platform,
            });
            ...
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 401) {
                    Alert.alert("Authentication Error", "Please log in again to share this post.");
