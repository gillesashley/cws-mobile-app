        ...
        <ThemedView style={[styles.card, { backgroundColor: surfaceColor }]}>
            ...
            <Controller
                ...
                render={({ field }) => (
                    <Picker onValueChange={field.onChange} selectedValue={field.value}>
                        {regions?.map((ct) => <Picker.Item key={ct.id} label={ct.name} value={ct.id} />)}
                    </Picker>
                )}
            />
            ...
            <Controller
                ...
                render={({ field }) => (
                    <Picker onValueChange={field.onChange} selectedValue={field.value}>
                        {constituencies
                            ?.filter((ct) => +ct.region_id === +control._formValues.region_id)
                            ?.map((ct) => <Picker.Item key={ct.id} label={ct.name} value={ct.id} />)}
                    </Picker>
                )}
            />
            <Text>Region</Text>
